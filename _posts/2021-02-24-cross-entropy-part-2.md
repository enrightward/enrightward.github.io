...
In this post, I will write down a technical result that $$h_{p}(q)$$ is a convex function, we restrict $$p$$ and $$q$$ to the space of all PDFs such that no event in $$S$$ is impossible. That is, we assume $$p(x_{i})$$ and $$q(x_{i})$$ is never zero. This assumption is not very unrestrictive for machine learning purposes, where learnt zero probabilities are rare. It also allows us to use the _softmax_ parametrisation for $$q$$, explained below.
...
In the previous post, I noted that a common machine learning task is to update a parametrised PDF $$q$$ to more closely resemble an idealised PDF $$p$$ implicit in some data set, and illustrated with the example of language modelling. The above convexity result, combined with last post's proof that $$q \mapsto H(p, q)$$ achieves a minimum at $$q = p$$, says this task is achievable with gradient descent.
